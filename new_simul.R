...
for(j in 1:5){
  ...
  result <- tryCatch(
    {
      # Measure the time taken by my_function
      ...
      sgca2$time[j] <- time_taken
      sgca2$loss[[j]]=c(subdistance(ainit,a),subdistance(final,a))
      #ag
      #output  # Return the output to be used in the result variable
    },
    error = function(e) {
      cat("Error in iteration", j, "- skipping\n")
      sgca2$time[j] <- NA  # Store NA for failed iterations
      sgca2$loss[[j]] <- NA
      return(NULL)
    }
  )
  ...
  result <- tryCatch(
    {
      ...
      # Measure the time taken by my_function
      start_time <- proc.time()
      output <- rgcca(blocks,method="rgcca",scale=FALSE)
      end_time <- proc.time()
      time_taken <- end_time - start_time
      temp=c()
      # Store the timing result
      U_SVD <- rbind((output$a)$block1,(output$a)$block2,(output$a)$block3)
      USigmaU_t <- t(U_SVD) %*% Sigma_0 %*% (U_SVD)
      USigmaU_t_sqrt <- pracma::sqrtm(USigmaU_t)$B
      USigmaU_t_inv_sqrt <- solve(USigmaU_t_sqrt)
      U <- U_SVD %*% USigmaU_t_inv_sqrt
      temp[1]=subdistance(U[,1:r],a)
      rgcca2$time[j] <- time_taken
      rgcca2$loss[j] <- sum(temp)
      output  # Return the output to be used in the result variable
    },
    error = function(e) {
      cat("Error in iteration", j, "- skipping\n")
      rgcca2$time[j] <- NA  # Store NA for failed iterations
      rgcca2$loss[j] <- NA
      return(NULL)
    }
  )
  result <- tryCatch(
    {
      ...
      Sigma_0[idx1, idx1]<-S[idx1, idx1]
      Sigma_0[idx2, idx2]<-S[idx2, idx2]
      Sigma_0[idx3, idx3]<-S[idx3, idx3]
      #Measure the time taken by my_function
      start_time <- proc.time()
      output <- rgcca(blocks,method="sgcca",scale=FALSE)
      end_time <- proc.time()
      time_taken <- end_time - start_time
      temp=c()
      # Store the timing result
      ...
      temp[1]=subdistance(U[,1:r],a)
      # Store the timing result
      sgcca2$time[j] <- time_taken
      sgcca2$loss[j]=sum(temp)
      output  # Return the output to be used in the result variable
    },
    error = function(e) {
      cat("Error in iteration", j, "- skipping\n")
      sgcca2$time[j] <- NA  # Store NA for failed iterations
      sgcca2$loss[j] <- NA
      return(NULL)
    }
  )
  result <- tryCatch(
    {
      ...
      end_time <- proc.time()
      admm_cv_time[j] <- end_time-start_time  # Store NA for failed iterations
      admm$loss[j] <- subdistance(cv_fit[[j]]$fit_min$U[,1], a)
      
      start_time <- proc.time()
      fit <- admm_sgca(S, sigma0hat, cv_fit[[j]]$lambda_min, r,
                       rho = 1,
                       p_list = pp,
                       penalty = c("l1"),
                       ...
                       verbose = TRUE)
      end_time <- proc.time()
      admm$time[j] <- end_time-start_time
      output  # Return the output to be used in the result variable
    },
    error = function(e) {
      cat("Error in iteration", j, "- skipping\n")
  ...
  result <- tryCatch(
    {
      ...
      evalues <- result$d
      evectors <-result$u
      u_oracle <- (sqrt_m_sigma$Binv %*% as.matrix(evectors))
      ...
      oracle2$loss[j]=subdistance(u_oracle[,1],a[newid])
      ...
      SVD=svd(target_mat);
      U_SVD= SVD$u; lam_SVD=SVD$d
      lam_inv=diag(1/lam_SVD)
      U_canon=C_oracle%*%sqrt_m_sigma$B%*%U_SVD%*%lam_inv
      
      end_time <- proc.time()
      naive$time[j]=end_time - start_time
      ...
      end_time <- proc.time()
      oracle$time[j]=end_time-start_time
      oracle$loss[j]=subdistance(fit_admm_GT$U[,1],a)
      output  # Return the output to be used in the result variable
    },
    error = function(e) {
      cat("Error in iteration", j, "- skipping\n")
      ...
      oracle2$loss[j] <- NA
      oracle$time[j] <- NA  # Store NA for failed iterations
      oracle$loss[j] <- NA
      naive$time[j] <- NA  # Store NA for failed iterations
      naive$loss[j] <- NA
      return(NULL)
    }
...
x_values <- c(30, 1000, 10000, 50000, 100000)

# Combine time data into a single data frame
time_data <- data.frame(
  x = rep(x_values, 8),
  time = c(sgca2$time, rgcca2$time, sgcca2$time, admm$time, admm_cv_time, oracle$time, oracle2$time, naive$time),
  method = rep(c("SGCA (Gao)", "Tenenhaus Method", "Sparse Tenenhaus method", "ADMM", "ADMM CV", "Oracle","Oracle2", "Naive"), each = length(x_values))
)

# Combine loss data into a single data frame
sgca_loss1 <- c(sgca2$loss[[1]][1], sgca2$loss[[2]][1], sgca2$loss[[3]][1], sgca2$loss[[4]][1], sgca2$loss[[5]][1])
sgca_loss2 <- c(sgca2$loss[[1]][2], sgca2$loss[[2]][2], sgca2$loss[[3]][2], sgca2$loss[[4]][2], sgca2$loss[[5]][2])

loss_data <- data.frame(
  x = rep(x_values, 8),
  loss = c(sgca_loss1, sgca_loss2, rgcca2$loss, sgcca2$loss, admm$loss, oracle$loss, oracle2$loss, naive$loss),
  method = c(rep("Gao Fantope error", length(x_values)), rep("Gao final error", length(x_values)),
             rep("Tenenhaus method", length(x_values)), rep("Sparse Tenenhaus method", length(x_values)),
             rep("ADMM", length(x_values)), rep("Oracle", length(x_values)), rep("Oracle2", length(x_values)),rep("Naive", length(x_values))))

ggplot(time_data, aes(x = x, y = time, color = method, shape = method)) +
  geom_line() +
  geom_point(size = 3) +
  scale_x_continuous(trans = "log10") + # Optional: Log scale for x-axis
  labs(
...
ggplot(loss_data, aes(x = x, y = loss, color = method, shape = method)) +
  geom_line() +
  geom_point(size = 3) +
  scale_x_continuous(trans = "log10") + # Optional: Log scale for x-axis
  labs(
...
# 1) Drop SGCA (Gao) and ADMM CV from the time dataset
time_zoom <- subset(time_data, !method %in% c("SGCA (Gao)", "ADMM CV"))
time_zoom$method <- droplevels(factor(time_zoom$method))  # drop unused legend entries

# 2a) Simple zoom: let ggplot auto-scale to remaining data
ggplot(time_zoom, aes(x = x, y = time, color = method, shape = method)) +
  geom_line() +
  geom_point(size = 3) +
  scale_x_continuous(trans = "log10") +
  labs(title = "Time Comparison (zoomed, excluding SGCA (Gao) & ADMM CV)",
       x = "X-axis Points", y = "Time") +
  theme_minimal()

# 2b) Optional: set a tighter y-window (e.g., to the 1st–95th percentile of remaining times)
ylims <- quantile(time_zoom$time, probs = c(0.01, 0.95), na.rm = TRUE)
ggplot(time_zoom, aes(x = x, y = time, color = method, shape = method)) +
  geom_line() +
  geom_point(size = 3) +
  scale_x_continuous(trans = "log10") +
  coord_cartesian(ylim = ylims) +   # zooms without dropping data
  labs(title = "Time Comparison (zoomed, excluding SGCA (Gao) & ADMM CV)",
       x = "X-axis Points", y = "Time") +
  theme_minimal()
