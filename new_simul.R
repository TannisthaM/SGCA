...
for(experiment in 1:50){
  for(j in 1:5){
    ...
    result <- tryCatch(
      ...
      error = function(e) {
        ...
        sgca2$time.init[j] <- NA  # Store NA for failed iterations
    ...
    result <- tryCatch(
      {
        ...
        output <- rgcca(
          ...
          method     = "rgcca",          # plain RGCCA (no sparsity)
          verbose    = TRUE
        )
        end_time <- proc.time()
        ...
        temp=c()
        # Store the timing result
        U_SVD <- as.matrix(rbind((output$astar),(output$astar),(output$astar)))
        
        temp=subdistance(U_SVD[,1:r],a)
    ...
    result <- tryCatch(
      {
        ...
        output <- rgcca(
          ...
          scheme     = "factorial",      # default scheme
          scale      = TRUE,             # standardize variables
          method     = "sgcca",          # plain RGCCA (no sparsity)
          verbose    = TRUE
        )
        end_time <- proc.time()
        time_taken <- end_time - start_time
    ...
    result <- tryCatch(
      {
        ...
        fit <- admm_sgca(S, sigma0hat, cv_fit[[j]]$lambda_min, r,
                         ...
                         verbose = TRUE)
        end_time <- proc.time()
        admm$time[j] <- (end_time-start_time)[3]
        output  # Return the output to be used in the result variable
      },
      error = function(e) {
        cat("Error in iteration", j, "- skipping\n")
    ...
    result <- tryCatch(
      {
        ...
        end_time <- proc.time()
        oracle$time[j]=(end_time-start_time)[3]
  ...
  expmnts[[experiment]]=list(sgca=sgca2,rgcca=rgcca2,sgcca=sgcca2,admm=admm,
