#!/bin/bash
#SBATCH --job-name=egcar_p_cv
#SBATCH --output=logs/egcar_p_cv_%A_%a.out
#SBATCH --error=logs/egcar_p_cv_%A_%a.err
...
SCRIPT="$ROOT/r/experiments/cluster/run_egcar_vary_p_cv.R"
OUTDIR="$ROOT/egcar_vary_p_cv_outputs/job_${SLURM_ARRAY_JOB_ID}"
EXPECTED_TASKS=180

mkdir -p "$ROOT/logs" "$OUTDIR"/{metrics,cv,snapshots,plots,config}
cd "$ROOT"

# 18 configurations = 3 ranks x 6 values of p/block.
