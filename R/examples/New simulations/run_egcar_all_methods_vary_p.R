...
# =============================================================================
# EGCAR and multiview benchmark simulation: increasing block dimension at n=150
#
# Methods:
...
build_configs <- function() {
  out <- expand.grid(
    rank = c(1L, 2L, 5L),
    p_per_block = c(30L, 50L, 100L, 500L, 1000L, 5000L),
    KEEP.OUT.ATTRS = FALSE,
    stringsAsFactors = FALSE
  )
  out$n <- 150L
  out$panel <- "n=150"
  out <- out[order(out$rank, out$p_per_block),
