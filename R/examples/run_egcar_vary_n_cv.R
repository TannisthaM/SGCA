...
EXPERIMENT <- "vary_n_cv"
SCRIPT_NAME <- "run_egcar_vary_n_cv.R"
...
make_config_table <- function() {
  rows <- list()
  id <- 1L
  for (p in c(100L, 150L)) {
    n_grid <- if (p == 100L) {
      c(200L, 250L, 300L, 500L, 1000L, 10000L)
    } else {
      c(250L, 300L, 450L, 1000L, 5000L, 10000L)
    }
    for (rank in RANKS) {
      for (n in n_grid) {
        rows[[id]] <- data.frame(
