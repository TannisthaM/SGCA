...
# This script is chunk-safe.  The complete experiment contains 360 tasks.
# Submit six 60-task chunks with offsets 0,60,120,180,240,300 and the same
# EGCAR_RUN_ID.  A single full-array submission is also possible by overriding
# --array=1-360%5 and leaving EGCAR_TASK_OFFSET=0.
...
if (( global_task < 1 || global_task > 360 )); then
  echo "Invalid global task ${global_task}; expected 1..360" >&2
  exit 2
fi
...
# The last successful task to observe all 360 metric files performs aggregation.
expected=360
