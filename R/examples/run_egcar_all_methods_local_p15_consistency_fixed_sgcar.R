...
# =============================================================================
# Local consistency simulation for EGCAR and multiview benchmarks
#
# Design:
#   * K = 3 blocks.
#   * p_1 = p_2 = p_3 = 15.
#   * The first five variables in every block are active; all remaining rows
#     of the population precision-adjusted cross-view operator are zero.
#   * n in {30, 45, 100, 1000, 5000, 10000}.
#   * target rank r in {1, 2, 5}.
#   * exactly one simulated dataset for every (n, r) point.
#   * row-localization threshold tau = 1e-4.
#
# Methods:
#   Oracle1-population : true population Sigma and Sigma0 are used for pairwise
#                        C construction and the spectral loading step.
#   Oracle2-support    : true active rows are known, but C and loadings are
#                        estimated from the sample.
#   EGCAR-theory       : pairwise l1-ADMM with
#                        rho = sqrt(log(p_total) / n).
#   EGCAR-CV           : one common rho for all three pairwise blocks, selected
#                        by five-fold held-out generalized-Rayleigh CV over
#                        {1e-5, 1e-4, ..., 1, 10, ..., 1e4}.
#   SGCA               : Gao--Ma benchmark through the custom SGCAR package.
#   RGCCA              : regularized GCCA with unsupervised CV.
#   SGCCA              : sparse RGCCA with unsupervised CV.
#
# The SGCA/RGCCA/SGCCA methods natively return loading directions rather than
# the EGCAR operator. For a fair C-error comparison, C_{k,l} is reconstructed
# inside the estimated loading spans by solving the reduced pairwise normal
# equation in those spans.
#
# Population model: Section 6.1 of the manuscript.
#   Sigma_kk[i,j] = alpha^|i-j|, alpha = 0.30,
#   U_k = G_k (G_k' Sigma_kk G_k)^(-1/2),
#   Sigma_kl = signal_lambda Sigma_kk U_k U_l' Sigma_ll,
#   C0_kl = signal_lambda U_k U_l', signal_lambda = 0.80.
#
# The population is held fixed across n within each rank. Separate data
# realizations are generated for the six sample sizes.
#
# Basic use:
#   Rscript run_egcar_all_methods_local_p15_consistency.R
#
# Optional output directory:
#   Rscript run_egcar_all_methods_local_p15_consistency.R my_results
#
# Use a chosen number of local worker processes (at most five are useful for
# five-fold CV):
#   EGCAR_LOCAL_CORES=5 Rscript run_egcar_all_methods_local_p15_consistency.R
#
# Resume behavior: completed (n,r) points are skipped. To overwrite them:
#   EGCAR_OVERWRITE=1 Rscript run_egcar_all_methods_local_p15_consistency.R
#
# Package helpers:
#   Rscript run_egcar_all_methods_local_p15_consistency.R check_packages
#   Rscript run_egcar_all_methods_local_p15_consistency.R install_packages
# =============================================================================
...
SPECIAL_MODES <- c("check_packages", "install_packages", "help")
MODE <- if (length(ARGS) && ARGS[[1]] %in% SPECIAL_MODES) ARGS[[1]] else "run"
...
# The GitHub SGCAR package currently does not declare its runtime
# dependencies in DESCRIPTION, although several R files attach them at load
# time.  They must therefore be installed explicitly before SGCAR.
CRAN_PACKAGES <- c(
  "MASS", "Matrix", "future", "future.apply", "PMA", "pracma",
  "RGCCA", "RSpectra", "ggplot2", "remotes",
  "tidyverse", "tidyr", "dplyr", "geigen", "expm",
  "foreach", "doParallel", "crayon"
)
...
sgcar_is_usable <- function() {
  if (!requireNamespace("SGCAR", quietly = TRUE)) return(FALSE)
...
install_required_packages <- function() {
  ...
  if (!sgcar_is_usable()) {
    ...
    # Repository name differs from the package name: repo SGCA, package SGCAR.
    github_repo <- Sys.getenv("SGCAR_GITHUB_REPO", "TannisthaM/SGCA")
    ...
    if (!sgcar_is_usable()) {
      stop(
        "SGCAR was installed but the required internal functions were not ",
        "found. Remove any stale SGCAR installation and reinstall from ",
        "TannisthaM/SGCA@main."
      )
    }
  }
...
check_required_packages(stop_on_missing = identical(MODE, "run"))

suppressPackageStartupMessages({
  # These are attached because the uploaded benchmark workflow attaches them
  # before calling the custom SGCAR internals.
  library(SGCAR)
  library(MASS)
  library(Matrix)
...
CFG <- list(
  ...
  admm_max_iter = get_env_int("EGCAR_ADMM_MAX_ITER", 2000L),
  admm_abs_tol = get_env_num("EGCAR_ADMM_ABS_TOL", 1e-5),
  admm_rel_tol = get_env_num("EGCAR_ADMM_REL_TOL", 1e-4),
  ...
  base_seed = get_env_int("EGCAR_BASE_SEED", 8242026L)
)
...
N_REPS <- 1L
EXPERIMENT <- "local_p15_vary_n"

build_configs <- function() {
  out <- expand.grid(
    rank = c(1L, 2L, 5L),
    n = c(30L, 45L, 100L, 1000L, 5000L, 10000L),
    KEEP.OUT.ATTRS = FALSE,
    stringsAsFactors = FALSE
  )
  out$p_per_block <- 15L
  out$panel <- "p1=p2=p3=15"
  out <- out[order(out$rank, out$n),
...
available_cores <- function() {
  requested <- suppressWarnings(as.integer(Sys.getenv("EGCAR_LOCAL_CORES", "")))
  if (!is.finite(requested) || requested < 1L) {
    requested <- suppressWarnings(as.integer(Sys.getenv("EGCAR_CV_CORES", "")))
  }
  if (!is.finite(requested) || requested < 1L) {
    detected <- parallel::detectCores(logical = FALSE)
    if (!is.finite(detected) || detected < 1L) detected <- 1L
    requested <- max(1L, detected - 1L)
  }
  # The principal parallel unit is a five-fold CV split.
  min(CFG$cv_folds, max(1L, requested))
}

parallel_lapply_local <- function(X, FUN, cores = 1L) {
  cores <- min(length(X), max(1L, as.integer(cores)))
  if (cores <= 1L || length(X) <= 1L) return(lapply(X, FUN))

  if (.Platform$OS.type == "unix") {
    return(parallel::mclapply(
      X, FUN, mc.cores = cores, mc.set.seed = FALSE
    ))
  }

  # Windows has no multicore fork. future.apply supplies a multisession
  # implementation and exports the functions/data used by FUN automatically.
...
fit_pair_blocks <- function(X, candidate_sets, rho, cores = 1L, warm_blocks = NULL) {
  ...
  fits <- parallel_lapply_local(
    seq_len(nrow(pair_index)), fit_one, cores = ncores
  )
...
select_egcar_rho_cv <- function(X_raw, pop, rank, cores) {
  ...
  pieces <- parallel_lapply_local(
    seq_len(folds), worker, cores = ncores
  )
...
direct_rgcca_unsupervised_cv <- function(blocks, method, rank, cores, seed) {
  ...
  pieces <- parallel_lapply_local(
    folds, fold_worker, cores = ncores
  )
  detail <- do.call(rbind, pieces)
...
run_worker <- function(config_id, rep_id, outdir) {
  ...
  cat(sprintf(
    "%s: point %d/%d, run %d, n=%d, p/block=%d, r=%d, workers=%d\n",
...
plot_metric <- function(S, rank_value, xvar, metric, ylab, path,
                        facet = FALSE, log_x = TRUE, log_y = TRUE,
                        add_rate = FALSE) {
  ...
  p <- ggplot(
    D,
    aes_string(
      x = xvar, y = "plot_value", color = "method",
  ...
  if (N_REPS > 1L) {
    p <- p + geom_errorbar(
      aes(ymin = lower, ymax = upper), width = 0,
  ...
  p <- p + labs(
      ...
      caption = paste(
        "One simulated dataset per (n,r) point.",
        if (log_y) "Numerical zeros are displayed at 1e-12." else ""
      )
    ) +
...
aggregate_results <- function(outdir) {
  ...
  atomic_csv(S, file.path(outdir, "summary_one_experiment_per_point.csv"))
  ...
  for (r in c(1L, 2L, 5L)) {
    ...
    pdf(file.path(outdir, "plots", sprintf("C_consistency_rank_%d.pdf", r)),
        width = 8.5, height = 5.8, onefile = TRUE)
    for (p in plist[1:4]) print(p)
    dev.off()

    pdf(file.path(outdir, "plots", sprintf("canonical_consistency_rank_%d.pdf", r)),
        width = 8.5, height = 5.8, onefile = TRUE)
    for (p in plist[5:7]) print(p)
    dev.off()
  }
  ...
  names(status)[names(status) == "rep_id"] <- "successful_runs"
  atomic_csv(status, file.path(outdir, "successful_runs.csv"))

  writeLines(
    c(
      paste("Experiment:", EXPERIMENT),
      paste("Completed points found:", length(files), "of expected", EXPECTED_TASKS),
...
# ------------------------ optional consistency heatmaps ----------------------

heat_colors <- grDevices::colorRampPalette(
  c("#2166AC", "#F7F7F7", "#B2182B")
)(101L)

plot_matrix_heatmap <- function(M, title, zlim, block_size = 15L,
                                draw_block_lines = TRUE) {
  if (is.null(M) || !is.matrix(M) || !all(is.finite(M))) {
    plot.new()
    graphics::title(main = title)
    text(0.5, 0.5, "Method failed or matrix unavailable")
    return(invisible(NULL))
  }

  nr <- nrow(M)
  nc <- ncol(M)
  z <- t(M[nr:1L, , drop = FALSE])
  image(
    x = seq_len(nc), y = seq_len(nr), z = z,
    col = heat_colors, zlim = zlim,
    axes = FALSE, xlab = "Column", ylab = "Row", main = title,
    useRaster = TRUE
  )
  axis(1, at = pretty(seq_len(nc)), labels = pretty(seq_len(nc)), cex.axis = 0.7)
  axis(2, at = pretty(seq_len(nr)),
       labels = rev(pretty(seq_len(nr))), cex.axis = 0.7)
  box()

  if (draw_block_lines && nr == nc && nr %% block_size == 0L) {
    cuts <- seq(block_size + 0.5, nr - 0.5, by = block_size)
    abline(v = cuts, h = cuts, lty = 2, lwd = 0.7)
  } else if (nr %% block_size == 0L) {
    cuts <- seq(block_size + 0.5, nr - 0.5, by = block_size)
    # Rows are reversed by image(); equal block sizes keep the same cut values.
    abline(h = cuts, lty = 2, lwd = 0.7)
  }
}

align_loading_to_truth <- function(L_hat, L0, Sigma0) {
  if (is.null(L_hat) || !is.matrix(L_hat) ||
      !all(dim(L_hat) == dim(L0)) || !all(is.finite(L_hat))) {
    return(NULL)
  }
  cross <- crossprod(L_hat, Sigma0 %*% L0)
  sv <- svd(cross, nu = ncol(L0), nv = ncol(L0))
  rotation <- sv$u %*% t(sv$v)
  L_hat %*% rotation
}

snapshot_path <- function(outdir, config_id, rep_id = 1L) {
  file.path(
    outdir, "fits",
    sprintf("fit_cfg%03d_rep%02d.rds", config_id, rep_id)
  )
}

plot_consistency_heatmaps <- function(outdir) {
  enabled <- tolower(Sys.getenv("EGCAR_SAVE_HEATMAPS", "1")) %in%
    c("1", "true", "yes", "y")
  if (!enabled) return(invisible(NULL))

  methods <- METHOD_LEVELS
  for (rank_value in c(1L, 2L, 5L)) {
    ids <- CONFIGS$config_id[CONFIGS$rank == rank_value]
    ids <- ids[order(CONFIGS$n[match(ids, CONFIGS$config_id)])]

    c_pdf <- file.path(
      outdir, "plots", sprintf("C_matrix_heatmaps_rank_%d.pdf", rank_value)
    )
    grDevices::pdf(c_pdf, width = 15.5, height = 8.4, onefile = TRUE)
    for (config_id in ids) {
      cfg <- CONFIGS[CONFIGS$config_id == config_id, , drop = FALSE]
      path <- snapshot_path(outdir, config_id)
      if (!file.exists(path)) next
      obj <- readRDS(path)
      mats <- c(
        list(`Ground truth C0` = obj$full_matrices$C0),
        lapply(methods, function(m) {
          z <- obj$full_matrices[[m]]
          if (is.null(z)) NULL else z$C
        })
      )
      names(mats) <- c("Ground truth C0", methods)
      finite_max <- max(vapply(mats, function(M) {
        if (is.null(M) || !length(M)) return(NA_real_)
        max(abs(M), na.rm = TRUE)
      }, numeric(1)), na.rm = TRUE)
      if (!is.finite(finite_max) || finite_max <= 0) finite_max <- 1

      par(mfrow = c(2, 4), mar = c(2.7, 2.7, 3.1, 1.2), oma = c(0, 0, 2, 0))
      for (nm in names(mats)) {
        plot_matrix_heatmap(mats[[nm]], nm, c(-finite_max, finite_max))
      }
      mtext(
        sprintf("Reconstructed C matrices: r=%d, n=%d", rank_value, cfg$n),
    ...
    l_pdf <- file.path(
      outdir, "plots", sprintf("canonical_direction_heatmaps_rank_%d.pdf", rank_value)
    )
    grDevices::pdf(l_pdf, width = 15.5, height = 8.4, onefile = TRUE)
    for (config_id in ids) {
      cfg <- CONFIGS[CONFIGS$config_id == config_id, , drop = FALSE]
      path <- snapshot_path(outdir, config_id)
      if (!file.exists(path)) next
      obj <- readRDS(path)
      L0 <- obj$full_matrices$L0
      Sigma0 <- obj$full_matrices$Sigma0
      mats <- list(`Ground truth L0` = L0)
      for (m in methods) {
        z <- obj$full_matrices[[m]]
        mats[[m]] <- if (is.null(z)) NULL else
          align_loading_to_truth(z$L, L0, Sigma0)
      }
      finite_max <- max(vapply(mats, function(M) {
        if (is.null(M) || !length(M)) return(NA_real_)
        max(abs(M), na.rm = TRUE)
      ...
      for (nm in names(mats)) {
        plot_matrix_heatmap(
          mats[[nm]], nm, c(-finite_max, finite_max),
          block_size = 15L, draw_block_lines = FALSE
        )
      }
      mtext(
        sprintf(
          "Canonical loading directions after Sigma0-Procrustes alignment: r=%d, n=%d",
          rank_value, cfg$n
        ),
        outer = TRUE, cex = 1.08, font = 2
      )
    }
    grDevices::dev.off()
  }
...
run_local_experiment <- function(outdir) {
  make_output_dirs(outdir)
  overwrite <- tolower(Sys.getenv("EGCAR_OVERWRITE", "0")) %in%
    c("1", "true", "yes", "y")

  writeLines(
    c(
      paste("Experiment:", EXPERIMENT),
      "K=3; p1=p2=p3=15; first five rows active in each block",
      paste("n grid:", paste(sort(unique(CONFIGS$n)), collapse = ", ")),
      paste("rank grid:", paste(sort(unique(CONFIGS$rank)), collapse = ", ")),
      paste("tau:", format(CFG$row_tau, scientific = TRUE)),
      paste("signal lambda:", CFG$signal_lambda),
      paste("Toeplitz alpha:", CFG$alpha),
      paste("EGCAR CV grid:", paste(format(RHO_GRID, scientific = TRUE), collapse = ", ")),
      paste("local workers:", available_cores()),
      paste("R:", R.version.string),
      paste("library paths:", paste(.libPaths(), collapse = " | "))
    ),
    file.path(outdir, "simulation_config.txt")
  )

  for (config_id in CONFIGS$config_id) {
    tag <- sprintf("cfg%03d_rep%02d", config_id, 1L)
    metric_file <- file.path(outdir, "metrics", paste0("metrics_", tag, ".csv"))
    fit_file <- file.path(outdir, "fits", paste0("fit_", tag, ".rds"))

    if (!overwrite && file.exists(metric_file) && file.exists(fit_file)) {
      cfg <- CONFIGS[CONFIGS$config_id == config_id, , drop = FALSE]
      message(
        "Skipping completed point: r=", cfg$rank, ", n=", cfg$n,
        ". Set EGCAR_OVERWRITE=1 to refit."
      )
      next
    }
    run_worker(config_id, 1L, outdir)
  }

  aggregate_results(outdir)
  plot_consistency_heatmaps(outdir)
  message("All local results are in: ", normalizePath(outdir, mustWork = FALSE))
}
...
usage <- function() {
  cat(
    ...
    "  EGCAR_LOCAL_CORES=5     Number of CV workers (maximum useful value 5).\n",
    "  EGCAR_OVERWRITE=1       Refit completed (n,r) points.\n",
    "  EGCAR_SAVE_HEATMAPS=0   Disable C/loading heatmap PDFs.\n",
    "  SGCAR_LOCAL_SOURCE=...  Local SGCAR package source for installation.\n",
    "  SGCAR_GITHUB_REPO=...   owner/repository for SGCAR installation.\n",
    sep = ""
  )
}
...
if (identical(MODE, "run")) {
  output_dir <- if (length(ARGS) >= 1L) ARGS[[1]] else
    "egcar_all_methods_local_p15_results"
  run_local_experiment(output_dir)
} else if (identical(MODE, "help")) {
  usage()
} else {
  # check_packages and install_packages exit earlier in the script.
  usage()
}
